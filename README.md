...
Enthaltene Sheets: `git`, `regex`, `docker`, `ddev`, `composer`, `typo3`, `shopware`, `oxid`, `vim`, `lazyvim`, `nano`, `yazi`, `screen`. Neue Sheets als `cheatsheets/sheets/<name>.md` ablegen und in `bootstrap/remote.sh` (`CHEAT_SHEETS`) ergänzen.
